...
\item Let $m$ be odd, and suppose $p\nmid uv$ and $\chi_p(uv)=-1$. Then $T_\alpha(u,v)=0$ for every $\alpha\ge1$.
...
(b) For $\alpha=1$, Sali\'e's evaluation gives $T_1(u,v)=\chi_p(v)\,\mathfrak g_p\sum_{x^2\equiv4uv}e(x/p)$, and the sum is empty. For $\alpha\ge2$, put $\beta=\lfloor\alpha/2\rfloor\ge1$ and write $d=d_0(1+p^{\alpha-\beta}y)$ with $y\bmod p^\beta$. Then $\chi_p(d)=\chi_p(d_0)$, and $\bar d\equiv\bar d_0(1-p^{\alpha-\beta}y)\pmod{p^\alpha}$. Summing over $y$ gives $p^\beta$ times the indicator of $ud_0\equiv v\bar d_0\pmod{p^\beta}$, that is, of $ud_0^2\equiv v$. This forces $\chi_p(uv)=1$. So the sum is $0$.
...
Every $s_T(h;n)$ lies in $\Q(\zeta_{M},\sqrt{t/d})$ with $M=24k$. For an integer $g$ prime to $6tk$, let $\sigma_g$ be an automorphism of $\overline{\Q}$ with $\sigma_g(\zeta_M)=\zeta_M^{\,g}$; it acts on $\sqrt{t/d}$ by a sign. By Lemma~\ref{lem:sunit}, the moduli within one equal-growth group differ only by primes of $S$.
