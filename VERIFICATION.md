...
## Round 6 — independent step-by-step verification (Sept 16, 2026)

Every named lemma and proposition re-tested from independent implementations, each step
isolated so that compensating errors between steps cannot hide.

| piece | test | result |
|---|---|---|
| Zuckerman normalisation | growth-rate convergence to sqrt(mu)/(t sqrt 24) | consistent |
| Prop 3.x period divisibility | 7596 (t,psi,g,s) + exhaustive sub-claim | 0 failures |
| Lemma 5.2 Step 1 (omega identity, both parities of k) | 9159 (h,k,q) triples | 0 failures |
| Lemma 5.2 Step 2 (compatible lift, l even when 2 nmid t/d) | 1397 (t,d,K) triples | 0 failures |
| Lemma 5.2 Step 3 (D*/q = (t/d)/q) | 273 pairs + 6 Gauss-sum cases | 0 failures |
| Lemma 5.2 phase collection | 6120 (t,d,m,Lambda), exact rational | 0 failures |
| Prop 5.3 Kloosterman non-vanishing | 20477 (q,a,b), q prime <= 61; Weil bound | 0 failures |
| Prop 5.4 local lemma, q=2 | 7680 cases, B = 9..13 (moduli 512..8192), all 4 characters mod 8, all 8 theta-twists | 0 failures |
| Prop 5.4 local lemma, odd q | 1464 in-range cases, q = 3,5,7 | 0 failures |
| A_q threshold necessity | 18 breaks found strictly BELOW the stated threshold | confirms the hypothesis is sharp, not decorative |
| Lemma 6.3 no collisions | 90000 quadruples | 0 failures |
| Thm 1.1 ordering/termination | beta_0 = beta * (removed primes) >= beta | structural, holds |

### Extended end-to-end test
Least sign period computed for 22 pairs with 14 <= t <= 27, 24 < m(t-1) <= 72 — entirely
outside the published range. Least period is exactly t in all 22.

Stabilisation thresholds grow with t: t-periodic from n = 799 (18,3), 1170 (20,2),
1351 (22,2), 1491 (27,2), 2082 (24,2). A window beginning before the threshold shows no
period; seven such apparent "failures" at N=1600 all resolved on a later window.

### What this does NOT establish
Each step is individually correct as stated. That is not the same as each step FOLLOWING
from what precedes it. No test constructed here can distinguish a true-but-unjustified step
from a justified one. The residual risk remains exactly what earlier rounds identified:
expert human review of section 5.

---

## Round 7 — close reading (Sept 16, 2026)

Two gaps found by READING, not by testing. Both are places where the written argument did
not establish what it claimed; the numerics were correct in both cases, so ~150,000 tests
could never have surfaced them.

### G3 (section 5) — the Galois action on sqrt(t/d) was prescribed, not computed
The text said sigma_g "acts on sqrt(t/d) by a sign" and Step 3 then asserted that sign
IS (D*/q). That is forced only when sqrt(t/d) lies in Q(zeta_M), M=24k.

Writing k=dK with gcd(K,t/d)=1, the primes of D divide t/d and are prime to K, so
D* | M  <=>  D* | 24d.  This FAILS for 43 cusp types with t <= 40, e.g. t=10, d=2, D=5.
There sqrt 5 is not in Q(zeta_M) and the sign is a free choice, not a computed value.

FIXED: sigma_qbar is now fixed by two requirements — sigma(zeta_M)=zeta_M^qbar and
sigma(sqrt D)=(D*/q) sqrt D. Existence, compatibility (Gauss sum when D*|M; linear
disjointness otherwise) and cross-cusp consistency (multiplicativity of the Kronecker
symbol) are all argued in the text.

NOTE: the first repair attempt was itself WRONG. It wrote (D*/qbar)=(D*/q) "because the
symbol is a quadratic character". That holds only when D*|M (then q qbar = 1 mod D*);
in the escaping case it fails — 331 counterexamples found. The symbol must be evaluated
at q, not qbar. Corrected; the restricted claim verified 606/606.

### G4 (section 3) — Salie converse used but not stated
The proof of Thm 1.2(b) needs chi_p(uv)=+1 => T_alpha != 0, not merely the vanishing
direction. Verified 1108/1108. Lemma is now a biconditional with both directions proved:
for chi=+1 the Salie evaluation leaves 2 chi(v) g_p cos(2 pi x*/p), nonzero because
cos(2 pi x/p)=0 is impossible for integer x and odd p. Formula verified 0 failures over
p=5..19; smallest |cos| observed 0.0826.

### Additional stress tests this round
| claim | test | result |
|---|---|---|
| Lemma 3.x divisibility (4 terms, p^{2a-1} or c) | full sweep over p,alpha,c',a,b,d | 0 failures |
| Salie (b) vanishing, all alpha | 4004 cases, p=5..13, alpha=1..4 | 0 failures |
| Salie (c) Gauss sums, T_1(0,0)=0 | p=5,7,11 exact | holds |
| Thm 1.2(b) exceptional p=3, m=3 mod 6 | alpha(r) zeros, all m to 39 | exactly one class each |
| the m>=21 threshold | mu=m/12 > 4/3 iff m>16; next m=3 mod 6 is 21 | SHARP, not rounded |
| m=15 needs the (6,0) check | rate loses to k=6 only (0.1667 vs 0.1863) | confirms scope |
| Thm 1.2(b) degeneracy criterion | vs exact coefficients, p=5,7,11,13, m odd | 0 mismatches |

### Standing limitation
Rounds 1-6 established that every stated quantity is correct. Round 7 shows that is not
the same as every step being justified: two steps were true but unestablished. Sections
still not read at this level: section 2 setup, Corollary "decide", Lemma 5.1 (S-units).

---

## Round 8 — the even-t "normalisation gap" closes (Sept 16, 2026)

**Lemma 2.1 verified for even t, without numerics.** Direct evaluation of
F*(gamma tau) is infeasible (at c=576t the image has Im ~ 1e-7, so the eta product is at
|q|~1 and diverges — an earlier attempt "failed" for odd t too, which was the tell). The
Ligozat criterion settles it symbolically instead:
- sum delta*r_delta = 0 mod 24 and sum (N/delta)*r_delta = 0 mod 24: no failures for
  t=2..13, m=1..6.
- weight k = 0, character (s/d) with s = 24^m (24t)^{-m} = t^{-m}.
- (t^{-m}/d) = (t/d)^(m mod 2), trivial exactly when t is a perfect square (t=4,9).

**dictionary.py tests a stronger claim than the paper makes.** It matches single terms
across the two realisations at c=24k. That fails for even t (vanishing at c=24k for
t=4,6,10,12; irregular nonvanishing set — t=10 is nonzero at c=24,48,96 and zero at
c=120=24k). But Lemma 2.x matches the realisations RATE BY RATE. No term-by-term
dictionary is required anywhere in the proof.

**Status of the item flagged in round 7 as the main blocker: CLOSED.**

---

## Round 9 — machine verification (Sept 16, 2026)

Scripts in `code/machine/`. 15 goals, 15 discharged.

**Z3 (7 goals, UNSAT of the negation):** the Step 1 i-power identity; Step 2's
H = (t/d)h mod q and Kql' = Kl mod 24K; Step 3's two phase-collection coefficients;
Ligozat (i) and (ii).

**Exhaustive residue enumeration (8 goals, COMPLETE proofs — the statements depend only
on residues, so enumerating every class proves the universal statement):**
gcd(q,6)=1 => q^2 = 1 mod 24 (8 classes, gives 24 | k(q^2-1) for every k); qbar = q mod 8
(4 classes); 8 | (q-1)(q+1) (4 classes); Prop 3.x's unit divisibility (t<=200); the
character identity (t^{-m}/d) = (t/d)^(m mod 2); Salie (c); and cos(2 pi x/p) != 0.

Three of these were the goals Z3 returned UNKNOWN on — nonlinear modular arithmetic.
Residue enumeration closes them completely.

**NOT machine-verifiable with available tools:** the Rademacher-Zuckerman expansion and
its convergence, the uniqueness principle, Prop 5.4's stationary phase, Prop 5.3's
congruence argument, section 6's termination — and, centrally, whether each step FOLLOWS
from its predecessors. mathlib lacks eta multipliers, Kloosterman sums and the circle
method, so a Lean formalisation is not currently feasible.

Both defects found in this paper were of exactly the unverifiable kind.
